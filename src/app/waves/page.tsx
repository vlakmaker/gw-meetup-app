...
function Avatar({ name, photo_url, size = 44 }: {
  name: string;
  photo_url: string | null;
  size?: number;
}) {
  ...
  return (
    ...
    >
      <Link href={`/profile/${profile.id}`}>
        <Avatar name={profile.name} photo_url={profile.photo_url} />
      </Link>
      <div className="flex-1 min-w-0">
        ...
        {profile.work_one_liner && (
          <p className="text-text-secondary text-xs truncate">{profile.work_one_liner}</p>
        )}
      </div>
      <div className="flex flex-col items-end gap-1.5 shrink-0">
        <span className="text-text-secondary text-[10px]">{timeAgo}</span>
        {matched ? (
          <Link href="/connections" className="text-xs px-3 py-1 rounded-full font-medium"
            style={{ background: "var(--accent-primary)", color: "white" }}>
            🤝 Matched
          </Link>
        ) : waveBack ? (
          <button onClick={waveBack} disabled={waved}
  ...
  return (
    <div className="min-h-dvh pb-24">
      <div className="sticky top-0 z-10 px-4 pt-4 pb-3" style={{ background: "var(--bg-primary)" }}>
        <h1 className="font-mono text-xl font-bold mb-3">👋 Waves</h1>
      ...
      <div className="px-4 space-y-2">
        ...
        {!loading && displayList.length === 0 && (
          <div className="flex flex-col items-center py-16 gap-3 text-center">
            ...
            {tab === "sent" && (
              <Link href="/discover" className="text-sm px-4 py-2 rounded-xl font-medium"
                style={{ background: "var(--accent-primary)", color: "white" }}>
                Go to Discover
              </Link>
            )}
        ...
        {!loading && tab === "received" && received.map((w) => {
          ...
          return (
            <WaveRow key={w.id} profile={profile} time={w.created_at}
        ...
        {!loading && tab === "sent" && sent.map((w) => {
          ...
          const isMatched = matchedIds.has(profile.id);
          return (
            <WaveRow key={w.id} profile={profile} time={w.created_at} matched={isMatched} />
          );
        })}
      </div>
